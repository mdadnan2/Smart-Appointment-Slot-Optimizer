              ...
              {getFilteredAppointments().map((apt) => {
                ...
                return (
                  <tr key={apt.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${apt.status === 'CONFIRMED' ? 'bg-green-100 text-green-700' :
          ...
          return (
            <div key={apt.id} className="bg-white rounded-xl shadow-md p-4">
              <div className="flex items-start gap-3 mb-4">
                ...
                <span className={`px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap ${apt.status === 'CONFIRMED' ? 'bg-green-100 text-green-700' :
                    apt.status === 'PENDING' ? 'bg-yellow-100 text-yellow-700' :
                      apt.status === 'COMPLETED' ? 'bg-blue-100 text-blue-700' :
                        'bg-red-100 text-red-700'
                  }`}>
